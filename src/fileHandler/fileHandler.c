...
{
	static_file_config->files_path = config->web_interface.web_assets;
	static_file_config->url_prefix = NULL;

	static_file_config->mime_types = o_malloc(sizeof(struct _u_map));
	u_map_init(static_file_config->mime_types);
	u_map_put(static_file_config->mime_types, ".html", "text/html");
	u_map_put(static_file_config->mime_types, ".css", "text/css");
	u_map_put(static_file_config->mime_types, ".js", "application/javascript");
	u_map_put(static_file_config->mime_types, ".png", "image/png");
	u_map_put(static_file_config->mime_types, ".jpeg", "image/jpeg");
	u_map_put(static_file_config->mime_types, ".jpg", "image/jpeg");
	u_map_put(static_file_config->mime_types, "*", "application/octet-stream");

	ulfius_add_endpoint_by_val(instance, "GET", "*", NULL, 0, &callback_static_file, static_file_config);
}
...
int callback_static_file (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  if (!strncmp(request->http_url, "/v1", strlen("/v1"))) {
  	return U_CALLBACK_CONTINUE;
  }
